...
namespace RSELFANG.BO
{
    public class BOPqInpqr
    {
        ...
        public TOTransaction<List<GnItem>> getInfoTipificacion(int ite_cont)
        {
            DAOGnItems daoItems = new DAOGnItems();
            BOGnItems boItems = new BOGnItems();
            try
            {
                List<GnItem> dataTipi = new List<GnItem>();

                if (ite_cont != 0)
                {
                    dataTipi = daoItems.GetGnItemsFromTipi(330, ite_cont);
                }
                else
                {
                     dataTipi = boItems.GetGnItems(330);
                }
                
                return new TOTransaction<List<GnItem>>() { objTransaction = dataTipi, retorno = 0, txtRetorno = "" };
            }
            catch (Exception ex)
            {               
        ...
        public TOTransaction<GnFlag> getInfoflgTipi()
        {
            ...
            try
            {
                ...
                return new TOTransaction<GnFlag>() { objTransaction = spq000003, retorno = 0, txtRetorno = "" };
            }
            catch (Exception ex)
            {
                return new TOTransaction<GnFlag>() { objTransaction = null, retorno = 1, txtRetorno = ex.Message };
            }
