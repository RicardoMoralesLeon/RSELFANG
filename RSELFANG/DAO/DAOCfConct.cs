...
namespace RSELFANG.DAO
{
    public class DAOCfConct
    {
        public TOCfConct GetInfoCfConct(string ter_coda, int emp_codi)
        {
            ...
            sql.Append(" SELECT(SUM(SU_BONOC.BON_VALO) - SUM(SU_BONOC.BON_PAGO)) BON_SALD, MAX(DIM_FECH) DIM_FECH ");
        ...
        public List<ToSuDimco> GetInfoSuDimco(string ter_coda, int emp_codi, DateTime dim_feci, DateTime dim_fecf)
        {
            StringBuilder sql = new StringBuilder();
            List<SQLParams> sqlparams = new List<SQLParams>();
            sql.Append(" SELECT SU_DIMCO.DIM_FECH,SU_DIMCO.DIM_HOAU, SU_DIMCO.DIM_DTRA,SU_DIMCO.DIM_VTRA, ");
            ...
            sql.Append(" CASE WHEN SU_DPUPA.EMP_CODI IS NULL THEN GN_EMPRE.EMP_NOMB ELSE PO_PVDOR.PVR_NOCO END PVR_NOCO, SU_DPUPA.DPU_COPP, ");
            sql.Append(" SU_DPUPA.DPU_NOPP, SU_DIMCO.DIM_CTRL,SU_DIMCO.DIM_NURE, SU_IMCOD.IMC_OPRE,SU_DIMCO.DIM_DREA ");
            sql.Append(" FROM SU_IMCOD ");
